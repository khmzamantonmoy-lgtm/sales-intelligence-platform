      ...
      <div className="border-b border-gray-700 p-6">
        <div className="flex items-start justify-between mb-6">
          <div className="flex-1">
            ...
            <input className="block text-2xl font-bold text-white bg-transparent outline-none w-full border-b border-transparent hover:border-gray-600 focus:border-blue-500 transition-colors"
              value={deal.name} onChange={(e) => setDeal({ name: e.target.value })} placeholder="Deal name..." />
            <input className="block text-gray-400 text-sm bg-transparent outline-none w-full mt-1 border-b border-transparent hover:border-gray-700 focus:border-gray-500 transition-colors"
              value={deal.description} placeholder="Add deal description..." onChange={(e) => setDeal({ description: e.target.value })} />
          </div>
        </div>
        <div className="grid grid-cols-4 gap-4">
          <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
            ...
            <div className="flex items-center gap-1">
              ...
              <input type="number" className="text-green-400 text-lg font-bold bg-transparent outline-none w-full"
                value={deal.value} onChange={(e) => setDeal({ value: Number(e.target.value) })} />
            </div>
          </div>
          <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
            <div className="text-gray-500 text-xs uppercase tracking-wider mb-1">CURRENT STAGE</div>
            <select className="text-blue-400 text-lg font-bold bg-transparent outline-none w-full cursor-pointer"
              value={deal.stage} onChange={(e) => setDeal({ stage: e.target.value })}>
          ...
          <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
            ...
            <input type="date" className="text-white text-sm font-bold bg-transparent outline-none w-full"
              value={deal.closeDate} onChange={(e) => setDeal({ closeDate: e.target.value })} />
          </div>
          <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
          ...
          <div>
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-xl p-4">
              ...
              <div className="flex gap-2">
                <input className="flex-1 bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-500 outline-none"
                  placeholder="Name" value={ns.name} onChange={(e) => setNs({...ns, name: e.target.value})} />
                <input className="flex-1 bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-500 outline-none"
                  placeholder="Role" value={ns.role} onChange={(e) => setNs({...ns, role: e.target.value})} />
                <select className="bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-sm text-white outline-none"
                  value={ns.sentiment} onChange={(e) => setNs({...ns, sentiment: e.target.value})}>
                  <option>Champion</option><option>Supportive</option><option>Neutral</option><option>Skeptic</option>
                </select>
                <button onClick={() => {
                  if (!ns.name) return
                  setDeal({ stakeholders: [...deal.stakeholders, ns] })
                  setNs({ name: "", role: "", sentiment: "Neutral" })
                }} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-500">Add</button>
              </div>
            </div>
          </div>
        )}
